...
- Add a TSDoc/JSDoc comment to every exported function in `db/` and `src/lib/`. Describe the function's purpose, every parameter (including the injectable `db` argument), and the returned value. Keep the comment focused on public behaviour and constraints rather than repeating the implementation.
- Use comments for intent and non-obvious decisions only. If a comment merely paraphrases a query, expression, or control-flow statement, remove it.
- Update or remove comments whenever the related schema, helper, or transform changes; an outdated comment is a correctness defect.
